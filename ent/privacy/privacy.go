...
// The PersonaQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type PersonaQueryRuleFunc func(context.Context, *ent.PersonaQuery) error

// EvalQuery return f(ctx, q).
func (f PersonaQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.PersonaQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.PersonaQuery", q)
}

// The PersonaMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type PersonaMutationRuleFunc func(context.Context, *ent.PersonaMutation) error

// EvalMutation calls f(ctx, m).
func (f PersonaMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.PersonaMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.PersonaMutation", m)
}
